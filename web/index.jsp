...
<%
  int blockNum = 3;
  //当期所有版面标题
  String[] paperTitle = new String[]{"新闻","综合新闻","地方报道","社区科普","科技扶贫","法制农村","教育帮扶","生活百事"};

  //当前版面所有新闻标题
  String[] newsTitle = new String[]{"东坡区土地流转实现上网交易","华西广安医院成功开展首例腹腔镜下前列腺癌根治手术","积极创建“绿色基地"};
  //当前版面所有新闻详细内容
  ...
  String[] hotTitile = new String[]{"hotTitle1", "hotTitle2"};
  String[] hotAbstract = new String[]{"hotContent1", "hotContent2"};

  String[] readListTitle = new String[]{"readTitle1", "readTitle2"};
  String[] readListHref = new String[]{"href1", "href2"};
...
<div class="middle" id="middle">
  <div class="pdfImg">
    <img id="targetImg" width="400" height="550" src="page3.png"> </img>
  </div>
  <div class="midInfo">
    <div style="height: 30px;">
      <span class="now_page">第三版：<%=paperTitle[2]%></span>
    ...
    <div class="scrollPanel" id="pageContent" style="display: none;">
      ...
      <button class="returnBtn" onclick="switchPage2()" style="margin-left: 20px;width: 120px;">返回</button>
    </div>
  </div>
  <div class="leftNav">
    <div style="height: 30px;">
      <span class="pageTitle">版面目录</span>
    </div>
    <div class="realNav">
      <ul>
        <li>
          <a>第一版：<%=paperTitle[0]%></a>
        </li>
        <li>
          <a>第二版：<%=paperTitle[1]%></a>
        </li>
        <li>
          <a class="nowShowing">第三版：<%=paperTitle[2]%></a>
        </li>
        <li>
          <a>第四版：<%=paperTitle[3]%></a>
        </li>
        <li>
          <a>第五版：<%=paperTitle[4]%></a>
        </li>
        <li>
          <a>第六版：<%=paperTitle[5]%></a>
        </li>
        <li>
          <a>第七版：<%=paperTitle[6]%></a>
        </li>
        <li>
          <a>第八版：<%=paperTitle[7]%></a>
        </li>
      </ul>
    </div>
  </div>
  <div class="hotInfo">
    <div style="height: 30px;">
      <span class="pageTitle" style="width: 780px">本期热点</span>
    </div>
    <div class="scrollPanel">
      <ul>
        <% for(int i=0; i<hotTitile.length; i++){%>
          <li>
            <span><%=hotTitile[i]%></span>
            <p><%=hotAbstract[i]%></p>
          </li>
        <%}%>
      </ul>
  ...
  <div class="leftNav" style="margin-top:10px;">
    <div style="height: 30px;">
      <span class="pageTitle">阅读排行</span>
    </div>
    <div class="realNav">
      <ul>
        <% for(int i=0; i<readListTitle.length; i++){%>
          <li>
            <a href="<%=readListHref[i]%>"><%=readListTitle[i]%></a>
          </li>
        <%}%>
      </ul>
    </div>
...
<script>
  function switchPage(type){
    ...
    var newsContent=[];
    <% for(int i=0;i<blockNum;i++){%>
      newsTitle.push("<%=newsTitle[i]%>");
      newsContent.push("<%=newsContent[i]%>");
    <%}%>
...
//        }
  ...
  document.onclick=function(ev){
    var oEvent=ev||event;
    var x = oEvent.clientX - document.getElementById("middle").offsetLeft;
    var y = oEvent.clientY - document.getElementById("middle").offsetTop;
    //
    //
    //左边
    var blockNum = 2;
    var posXArray = [[0,200],[0,200]];
    var posYArray = [[0,140],[140,321]];
