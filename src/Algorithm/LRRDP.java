...
public class LRRDP {
    ...
    public LRRDP(int p, FileInfo fileInfo, FileOperation ufop,int logicBlockSize){
        pNum = p;
        this.realColNum = p+2;
        this.realRowNum = p-1;
        this.fileInfo = fileInfo;
        this.logicBlockSize=logicBlockSize;
        this.bandWidth = new int[pNum+2];
        ...
        zeroNum = (int)(realSize%(logicBlockSize*(pNum-1)*(pNum-1)));
                ...
                // lost data disk in the front part
                    ...
                    for(int s=0; s<stripeNum; s++) {
                        for (int i = 0; i < (pNum - 1); i++) {
                            ...
                            byte[] localRed = ReadFromFile(s, i, pNum + 1);
                            for (int k = 0; k < logicBlockSize; k++) {
                    ...
                    //two disks in the front part
                        ...
                        for(int s=0; s<stripeNum; s++) {
                            ...
                            for (int i = 0; i < (pNum - 1); i++) {
                                ...
                                // xor the local redundant
                                byte[] bs = ReadFromFile(s, i, pNum + 1);
                                for (int k = 0; k < logicBlockSize; k++) {
                                    tempData[k] = (byte) (tempData[k] ^ bs[k]);
                                }
