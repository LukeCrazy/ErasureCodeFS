...
public class Replication {
    ...
    public Replication(int rpl, FileInfo fileInfo, FileOperation ufop,int logicBlockSize){
        this.rplNum = rpl;
...
//                byte[] bs = new byte[logicBlockSize];
    ...
    public boolean decoding(int [] errPos) throws Exception{
        boolean res = false;
        if(errPos.length<=0 || errPos.length>rplNum){
            ...
            for(int i=0; i<errPos.length; i++){
                for(int j=0; j<blockNum; j++){
                    ...
                    System.out.println(logicBlockSize+"   "+j+"   "+errPos[0]);
