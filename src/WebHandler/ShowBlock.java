...
public class ShowBlock extends HttpServlet {
    ...
    public void doPost(HttpServletRequest req, HttpServletResponse res) {
        ...
        for (FileInfo item : fileList) {
            ...
            if (item.getFileName().equals(fileName) && item.getUserName().equals(count)) {
                ...
                switch (algName) {
                    case "replication":
                        Replication rpl = new Replication(algInfo.getParamK(), item, fop, item.getLogicBlockSize());
                        ...
                        break;
                    case "EVENODD":
                        EVENODD evenodd = new EVENODD(algInfo.getParamK(), item, fop, item.getLogicBlockSize());
                        ...
                        if (!(de_blockName.equals(""))) {
                            try {
                                ...
                                for (int l = 0; l < evenodd.getStripeNum(); l++) {
                                    for (int k = 0; k < evenodd.getRealRowNum(); k++) {
                                        FileOperation.DeleteFile("/testdir/" + count + "_" + fileName + "_S" + l + "_" + k + "_" + num + ".txt");
                                    }
                                }
                            } catch (Exception e) {
                                e.printStackTrace();
                        ...
                        for (int i = 0; i < evenodd.getRealColNum(); i++) {
                            for (int l = 0; l < evenodd.getStripeNum(); l++) {
                                for (int j = 0; j < evenodd.getRealRowNum(); j++) {
                                    tempPath = "/testdir/" + count + "_" + fileName + "_S" + l + "_" + j + "_" + i + ".txt";
                            ...
                            System.out.println("存在？" + check[i]);
                        }
                        System.out.println("getStripeNum______:" + evenodd.getStripeNum());
                        ;
                        try {
                            JSONObject RESULT = new JSONObject();
                            JSONArray jsonArray = new JSONArray();
                        ...
                        break;
                    case "LREVENODD":
                        LREVENODD lrevenodd = new LREVENODD(algInfo.getParamK(), item, fop, item.getLogicBlockSize());
                        ...
                        break;
                    case "RDP":
                        RDP rdp = new RDP(algInfo.getParamK(), item, fop, item.getLogicBlockSize());
                        ...
                        if (!(de_blockName.equals(""))) {
                            try {
                                ...
                                for (int l = 0; l < rdp.getStripeNum(); l++) {
                        ...
                        System.out.println("getStripeNum______:" + rdp.getStripeNum());
                        try {
                            JSONObject RESULT = new JSONObject();
                            JSONArray jsonArray = new JSONArray();
                            JSONObject jsonObject = new JSONObject();
                        ...
                        break;
                    case "LRRDP":
                        LRRDP lrrdp = new LRRDP(algInfo.getParamK(), item, fop, item.getLogicBlockSize());
                        ...
                        if (!(de_blockName.equals(""))) {
                            try {
                                ...
                                for (int l = 0; l < lrrdp.getStripeNum(); l++) {
                        ...
                        System.out.println("getStripeNum______:" + lrrdp.getStripeNum());
                        try {
                            JSONObject RESULT = new JSONObject();
                            JSONArray jsonArray = new JSONArray();
                            JSONObject jsonObject = new JSONObject();
    ...
    public void recoverBlock(String count, String file, ArrayList<FileInfo> fileList, HttpServletRequest request) {
        ...
        switch (Method) {
            case "replication":
                ...
                try {
                    ...
                    long presTime = resTime = Math.abs(ptime2 - time1);
        ...
        ArrayList<OptInfo> optList = (ArrayList<OptInfo>) request.getSession().getAttribute("optList");
        optList.add(optInfo);
    }
}
